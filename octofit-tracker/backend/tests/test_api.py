...
class UserViewSetTestCase(APITestCase):
    def setUp(self):
        ...
        self.user = User.objects.create(
            name="Iron Man", email="ironman@marvel.com", team=self.team
        )
...
class WorkoutViewSetTestCase(APITestCase):
    def setUp(self):
        self.workout = Workout.objects.create(
            name="Hero HIIT", description="High intensity for heroes"
        )
...
class APIExtraValidationTestCase(APITestCase):
    def setUp(self):
        self.team = Team.objects.create(name="Validation Team")

    def test_create_team_missing_name_returns_400(self):
        url = reverse("team-list")
        data = {}  # missing required 'name'
        response = self.client.post(url, data)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_create_activity_without_user_returns_400(self):
        url = reverse("activity-list")
        data = {"type": "Cycling", "duration": 20}  # missing user/user_id
        response = self.client.post(url, data)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_create_user_with_invalid_team_returns_400(self):
        url = reverse("user-list")
        data = {"name": "Test", "email": "test@example.com", "team_id": 9999}
        response = self.client.post(url, data)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
